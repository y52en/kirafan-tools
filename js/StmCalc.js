...
function RemS(rank, gwatch, swatch, bwatch, cs, ex) {
  ...
  var rtn =
    Math.floor(
      (stamina * gwatch * 1 +
        Math.floor(stamina * 0.5) * swatch +
        Math.floor(stamina * 0.1) * bwatch) /
        cs
    ) * ex;
  return rtn;
}
...
function StmCalc_a(rank, gwatch, swatch, bwatch) {
  ...
  ex = 4000;
  tEX =
    RemS(rank, gwatch, swatch, bwatch, cs, ex).toLocaleString() +
    " (" +
    (RemS(rank, gwatch, swatch, bwatch, cs, ex) / ex).toLocaleString() +
    "回)";
  return tEX;
}
